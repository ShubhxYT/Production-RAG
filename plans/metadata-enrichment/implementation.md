...
- [ ] Create file `ingestion/enrichment.py`:
...
- [ ] `python -c "from ingestion.enrichment import enrich_chunks, enrich_document; print('OK')"` prints `OK`
- [ ] Quick integration test — enrich chunks from a staged document:
...
- [ ] Verify summaries accurately reflect chunk content and questions are answerable
...
- [ ] Replace the entire contents of `ingestion/cli.py` with:
...
- [ ] Update `ingestion/__main__.py` (no changes needed — it already calls `main()`):
...
- [ ] `python -m ingestion --help` shows both `ingest` and `enrich` subcommands
- [ ] `python -m ingestion enrich --help` shows `--input`, `--output`, `--model`, `--delay` options
- [ ] Run enrichment on staging documents:

```bash
python -m ingestion enrich --input staging/ --delay 1.0
```

- [ ] Verify enriched JSON: `python -c "from ingestion.staging import load_staged_document; from pathlib import Path; d = load_staged_document(list(Path('staging').glob('*.json'))[0]); c = d.chunks[0]; print('Summary:', c.summary); print('Keywords:', c.keywords); print('Questions:', c.hypothetical_questions)"`
- [ ] Run enrichment again and verify all chunks are skipped (already enriched)
- [ ] Verify old `ingest` still works: `python -m ingestion ingest --help`
...
- [ ] Create file `test/test_enrichment.py`:
